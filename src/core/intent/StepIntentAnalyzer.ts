...
export class StepIntentAnalyzer {
  analyze(stepText: string): StepIntent {
    ...
    if (enterMatch) {
      return { action: 'enter', value: enterMatch[1], target: enterMatch[2].trim(), confidence: 'high' };
    }

    const clickMatch = text.match(/^(?:click|press|tap)\s+(?:the\s+)?(.+)$/i);
    if (clickMatch) {
      return { action: 'click', target: clickMatch[1].trim(), confidence: 'high' };
    }

    const selectMatch = text.match(/^select\s+"([^"]+)"\s+from\s+(.+)$/i);
    if (selectMatch) {
      return { action: 'select', value: selectMatch[1], target: selectMatch[2].trim(), confidence: 'high' };
    }

    const checkMatch = text.match(/^check\s+(?:the\s+)?(.+)$/i);
    if (checkMatch) {
      return { action: 'check', target: checkMatch[1].trim(), confidence: 'high' };
    }

    const uncheckMatch = text.match(/^uncheck\s+(?:the\s+)?(.+)$/i);
    if (uncheckMatch) {
      return { action: 'uncheck', target: uncheckMatch[1].trim(), confidence: 'high' };
    }
    ...
    if (verifyVisibleMatch) {
      return { action: 'verifyVisible', target: verifyVisibleMatch[1].trim(), confidence: 'high' };
    }

    const verifyTextMatch = text.match(/^verify(?:\s+text)?\s+"([^"]+)"\s+(?:in|on|at)\s+(.+)$/i);
    if (verifyTextMatch) {
      return { action: 'verifyText', value: verifyTextMatch[1], target: verifyTextMatch[2].trim(), confidence: 'high' };
    }

    const navigateMatch = text.match(/^(?:navigate|go)\s+to\s+(.+)$/i);
    if (navigateMatch) {
      return { action: 'navigate', target: navigateMatch[1].trim(), confidence: 'high' };
    }
    ...
    return { action: 'click', target: text, confidence: 'low' };
  }
}
