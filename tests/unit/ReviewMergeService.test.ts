...
`;
  ...
  it('handles optional-chaining syntax homeData.validUser?.password', () => {
    // Spec generated by SpecGenerator uses `homeData.validUser?.password` with `?.`
    const spec = `await homePage.enterPassword(homeData.validUser?.password);`;
    const data: Record<string, unknown> = {};
    const result = reconcileSpecDataReferences(spec, 'homeData', data);
    expect(result).toHaveProperty('validUser');
    const validUser = result['validUser'] as Record<string, unknown>;
    expect(validUser['password']).toBe('');
  });

  it('handles mixed direct and optional-chaining patterns in the same spec', () => {
    const spec = `
      await loginPage.enterUsername(homeData.validUser.username);
      await loginPage.enterPassword(homeData.validUser?.password);
      await genericPage.fillInput(homeData.inputs?.searchTerm ?? '');
    `;
    const data: Record<string, unknown> = { validUser: { username: 'user1' } };
    const result = reconcileSpecDataReferences(spec, 'homeData', data);
    const validUser = result['validUser'] as Record<string, unknown>;
    expect(validUser['username']).toBe('user1');
    expect(validUser['password']).toBe('');
    // inputs is a top-level key accessed via optional chaining so it gets '' placeholder.
